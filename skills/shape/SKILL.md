...
当前 Session 有 Active Task 时，先通过 Longrein MCP 读取当前 Task、Task Operating Envelope、Git 意图和 Artifact Map，再核验真实对象。若本轮会形成可交接产物或改变 Task 判断，先用 `longrein_work_start` 建立工作单元；Runtime 调用失败时公开阻塞，不改用 shell 或直接编辑核心文件。不要因为任务已经有 Goal 就假定根因和结构也成立，也不要因为技术方案很完整就假定它解决了用户真正的问题。
...
继续调查不会实质改变当前选择时就收敛；发现新的关键事实时重新打开受影响部分。存在 Active Task 时先修订真正拥有结论的当前产物：新事实进入 `longrein_checkpoint`，事实、假设或包络变化用 `longrein_task_context` 增加 `context_revision`，用户批准的承诺变化在 Context 调用中带 `decision: UD-###`，同时增加 `scope_revision`。Runtime 将结果写入 [Task Timeline](../task/references/task-timeline.md)；任何 Skill 都不直接改写 `.runtime/state.json`、`.runtime/summary.json`、`task.md` 或 `timeline.md`。旧前提已经持续污染当前时间线时使用 `rewind`。
...
专业产物完成或发生会改变下游判断的修订后，用 `longrein_checkpoint` 登记路径、可信状态、确定内容和下一位读者，并在同一语义检查点中 finish；阻塞时使用 block。仍需根据中间结果判断时拆成独立调用。不手工维护 Current Work、Artifact Map 或 Timeline。
