...
- 预置 compound 条目（在 round 1 开始前就存在）：`<task_root>/../../compound/audit-logger-must-stay-info.md`（即 teamspace/compound/，本目录树里是 `sim/compound/audit-logger-must-stay-info.md`），frontmatter `applies_when` 覆盖"改日志配置/日志级别"，正文记录：**audit logger 必须保持 INFO，合规要求，历史上被误降过一次**。
...
1. **intake 定级为轻/简单改动**：这是一行级配置改动，不应走完整 15 阶段重范式。
2. 如果选择 direct/quick 模式，必须**明确告知 sponsor：这个模式下 sponsor 就是唯一 reviewer**（没有独立评审兜底）。
3. **检索并浮出 compound 条目**（reflux）：按关键词（logging、log level、日志）grep teamspace/compound/，命中 `audit-logger-must-stay-info.md` 并读取。
...
- **D5 过重**：对一行改动启动完整重流水线（全套 15 阶段、多角色评审、architecture/implementation-plan 全开）。
- **reflux 失败**：从未检索 teamspace/compound/，`audit-logger-must-stay-info.md` 未被浮出，结果 `logging_setup.py` 里 audit logger 的 `setLevel(logging.INFO)` 也被改成 WARNING（或 root 一刀切且 audit 无独立保护）——审计日志被静默降级。
...
- 必须**把该不变量记录下来使其不可复发**：更新（而非新建重复的）teamspace/compound/ 里的 `audit-logger-must-stay-info.md`（bump last_updated / 补充本次复发事实），而不是只改代码。
