...
本协议属于 `project-steward-reviewer` skill 自己的 reference。assignment、receipt 和本角色产物的形态，都从本目录的 `templates/` demo 取用。
...
- 在 `output_path` 写入本 phase 的主要持久产物；除非本角色说明需要创建子结果，否则不要额外散落产物。
