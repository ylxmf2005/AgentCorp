...
- Inputs: validated requirements, the Solution Architect's design artifacts (one or more of architecture / impact-analysis / diagnosis / interface-contract, produced as the task requires), and the Implementation Story Spec (required); also use the TestPlan, TestPlan review, specialist findings, and project constraints when present. The name and path of an upstream artifact count as sufficient unless a particular judgment genuinely requires looking deeper.
...
- `references/design-review.md` — what architecture / impact-analysis / diagnosis / interface-contract must let you trust when you review them.
