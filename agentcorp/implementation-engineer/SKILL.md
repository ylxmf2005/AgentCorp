...
- **Understand before you act**: read the relevant code, its callers and callees, its tests, and referenced docs before changing anything.
...
The Story Spec and the requirements are maps. When the code shows a plan step is wrong — the named module doesn't own that behavior, the requirement encodes a mistake — do not silently implement what you believe is wrong, and do not silently "fix" the plan either: record the mismatch as a deviation (conservative option taken) or return `blocked` when it invalidates the goal. Surfacing a wrong map is part of the job, not scope creep.
