...
teamspace/tasks/<task_id>/
  ...
  review/
    ...
    fix-result.md
    fix-records/
  walkthrough/                    # 可选：面向变更的教学 artifact（walkthrough 能力）
    <slug>.html
  verification/
    assignments/
...
4. 推荐的下一步: 一条清晰建议。
5. 可选跟进项: 按需列 2-4 条，例如关闭任务、创建跟进项、跑 `walkthrough`（sponsor 理解 + quiz gate）或 `change-detailed-walker`（在本地 forge 上逐 hunk 的审计评论）、再做一轮验证、捕获/审阅经验、回 gate 修改。
