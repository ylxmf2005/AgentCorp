...
发起人说“继续做”“不要为常规审批停下来”或“你自行判断”时，选择 `interaction:auto`，并视为持续有效的导航偏好。在 `task.md` 中记录一次；跳过工作流允许跳过的所有人工暂停，在已批准范围内继续可逆决策，并把非紧急汇报推迟到下一个有意义的检查点，不再反复追问。这个偏好不能授权伪造证据、越权访问、自审自批，或在没有单独知情确认时执行破坏性/不可逆动作；这些约束只冻结受影响的阶段跃迁。`interaction:gate` 则会在下文列出的每道人工门禁暂停，包括原本可在征询后跳过的门禁。
